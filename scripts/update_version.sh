...
update_version_plugin_xml() {
    ...
    sed -i.bak "s/<version>.*<\/version>/<version>$new_version<\/version>/" "$PLUGIN_XML" && rm -f "${PLUGIN_XML}.bak"
    printf "Done."
}
...
update_version_gradle_properties() {
    ...
    sed -i.bak "s/^pluginVersion[[:space:]]*=.*/pluginVersion=$new_version/" "$GRADLE_PROPERTIES" && rm -f "${GRADLE_PROPERTIES}.bak"
    printf "Done."
}
